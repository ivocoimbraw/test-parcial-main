...
function mapFlutterWidgetToComponentType(flutterWidget: string): string {
  const mapping: Record<string, string> = {
    ...
    Row: COMPONENT_TYPES.ROW,
    Column: COMPONENT_TYPES.COLUMN,
    ...
    TextFormField: COMPONENT_TYPES.INPUT,
...
function parseWidgetProperties(content: string, component: ComponentNode) {
  ...
  parseLayoutProperties(content, component)
  parseAppBarProperties(content, component)
  parseSliderProperties(content, component)
  parseTableProperties(content, component)
  parseFormProperties(content, component)
...
function parseTextFieldProperties(content: string, component: ComponentNode) {
  if (component.type === COMPONENT_TYPES.TEXT_FIELD || component.type === COMPONENT_TYPES.INPUT) {
...
function parseLayoutProperties(content: string, component: ComponentNode) {
  if (component.type === COMPONENT_TYPES.ROW || component.type === COMPONENT_TYPES.COLUMN) {
    const mainAxisMatch = content.match(/mainAxisAlignment:\s*MainAxisAlignment\.(\w+)/)
    if (mainAxisMatch) {
      component.properties.mainAxisAlignment = convertFlutterAlignment(mainAxisMatch[1])
    }

    const crossAxisMatch = content.match(/crossAxisAlignment:\s*CrossAxisAlignment\.(\w+)/)
    if (crossAxisMatch) {
      component.properties.crossAxisAlignment = convertFlutterAlignment(crossAxisMatch[1])
    }
  }
}

function parseAppBarProperties(content: string, component: ComponentNode) {
  if (component.type === COMPONENT_TYPES.APP_BAR) {
    const titleMatch = content.match(/title:\s*Text\s*$$\s*['"]([^'"]*)['"]\s*$$/)
    if (titleMatch) {
      component.properties.title = titleMatch[1]
    }

    component.properties.showBackButton = content.includes("leading:")
  }
}
...
function parseFormProperties(content: string, component: ComponentNode) {
  if (component.type === COMPONENT_TYPES.FORM) {
    // Try to extract form title from first Text widget
    const titleMatch = content.match(
      /Text\s*\(\s*['"]([^'"]*)['"]\s*,\s*style:\s*TextStyle\s*\([^)]*fontWeight:\s*FontWeight\.bold/,
    )
    if (titleMatch) {
      component.properties.title = titleMatch[1]
    }
